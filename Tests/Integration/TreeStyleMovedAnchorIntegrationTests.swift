...
final class TreeStyleMovedAnchorIntegrationTests: XCTestCase {
    ...
    // Ported from yorkie-js-sdk v0.7.18: describe('Tree.Style range starting after a merge-moved
    // child') in `tree_style_moved_anchor_test.ts` (yorkie-js-sdk#1329 "Recover style ranges
    // collapsed by a merge at the from anchor").
    @MainActor
    func test_styles_the_writer_insert_when_a_merge_reverses_the_range() async throws {
        try await withTwoClientsAndDocuments(self.description) { c1, d1, c2, d2 in
            // given
            try d1.update { root, _ in
                root.tree = JSONTree(initialRoot: JSONTreeElementNode(type: "r", children: [
            ...
            // when — d1 inserts an empty <p> after the second paragraph, then styles a range
            // starting after `c` and ending inside its own insert. On d2 the concurrent merge
            // moves `cd` behind the insert, so the resolved range collapses and would miss the
            // insert entirely.
            try d1.update { root, _ in try (root.tree as? JSONTree)?.edit(8, 8, JSONTreeElementNode(type: "p", children: [])) }
            try d1.update { root, _ in try (root.tree as? JSONTree)?.style(6, 9, ["bold": "x"]) }
            try d2.update { root, _ in try (root.tree as? JSONTree)?.edit(0, 5) }
            ...
            // then
            XCTAssertEqual((d1.getRoot().tree as? JSONTree)?.toXML(), "<r><p bold=\"x\"></p>cd</r>")
            XCTAssertEqual(d1.toSortedJSON(), d2.toSortedJSON())
        }
    }

    @MainActor
    func test_applies_removeStyle_to_the_writer_insert_on_both_replicas() async throws {
        try await withTwoClientsAndDocuments(self.description) { c1, d1, c2, d2 in
            // given
            try d1.update { root, _ in
                root.tree = JSONTree(initialRoot: JSONTreeElementNode(type: "r", children: [
            ...
            // when — same shape as above with removeStyle: the removal tombstone must
            // materialize on both replicas, not only on the writer.
            try d1.update { root, _ in try (root.tree as? JSONTree)?.edit(8, 8, JSONTreeElementNode(type: "p", children: [])) }
            try d1.update { root, _ in try (root.tree as? JSONTree)?.removeStyle(6, 9, ["bold"]) }
            try d2.update { root, _ in try (root.tree as? JSONTree)?.edit(0, 5) }
            ...
            // then
            XCTAssertTrue(d1.toSortedJSON().contains("{\"attributes\":{},\"children\":[],\"type\":\"p\"}"))
            XCTAssertEqual(d1.toSortedJSON(), d2.toSortedJSON())
        }
    }

    @MainActor
    func test_keeps_a_range_that_stays_ordered_away_from_the_insert() async throws {
        try await withTwoClientsAndDocuments(self.description) { c1, d1, c2, d2 in
            // given
            try d1.update { root, _ in
                root.tree = JSONTree(initialRoot: JSONTreeElementNode(type: "r", children: [
            ...
            // when — both anchors sit inside the merged paragraph, so the resolved range moves
            // with the merge and stays ordered. The recovery must not widen it onto the insert.
            try d1.update { root, _ in try (root.tree as? JSONTree)?.edit(8, 8, JSONTreeElementNode(type: "p", children: [])) }
            try d1.update { root, _ in try (root.tree as? JSONTree)?.style(6, 7, ["bold": "x"]) }
            try d2.update { root, _ in try (root.tree as? JSONTree)?.edit(0, 5) }
    ...
    @MainActor
    func test_keeps_a_reversed_range_away_from_an_insert_unknown_to_the_styler() async throws {
        let rpcAddress = "http://localhost:8080"
        let c1 = Client(rpcAddress)
        let c2 = Client(rpcAddress)
        let c3 = Client(rpcAddress)
        ...
        // d3's insert is unknown to d1's style, so the version-vector check keeps it unstyled
        // even when the recovered traversal passes it.
        try d1.update { root, _ in try (root.tree as? JSONTree)?.edit(8, 8, JSONTreeElementNode(type: "p", children: [])) }
        try d1.update { root, _ in try (root.tree as? JSONTree)?.style(6, 9, ["bold": "x"]) }
        try d2.update { root, _ in try (root.tree as? JSONTree)?.edit(0, 5) }
        try d3.update { root, _ in try (root.tree as? JSONTree)?.edit(8, 8, JSONTreeElementNode(type: "b", children: [])) }
        ...
        XCTAssertEqual(d1.toSortedJSON(), d2.toSortedJSON())
        XCTAssertEqual(d2.toSortedJSON(), d3.toSortedJSON())
        XCTAssertTrue(d1.toSortedJSON().contains("{\"attributes\":{\"bold\":\"x\"},\"children\":[],\"type\":\"p\"}"))
        XCTAssertTrue(d1.toSortedJSON().contains("{\"children\":[],\"type\":\"b\"}"))
