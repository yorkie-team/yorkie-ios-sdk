...
class DocumentTests: XCTestCase {
    ...
    func test_change_paths_test_for_object() async throws {
        ...
        await target.subscribe("$.") { _, _ in
            let array = try? target.getValueByPath("$.") as? JSONObject
    ...
    func test_should_publish_snapshot_event_with_up_to_date_document() async throws {
        try await withTwoClientsAndDocuments(self.description) { c1, d1, c2, d2 in
            ...
            try await eventCollector.waitAndVerifyNthValue(milliseconds: 300,
                                                           at: 1,
                                                           isEqualTo: Int32(defaultSnapshotThreshold + 1))
        }
    }
}
